...
export async function POST(request: NextRequest) {
  ...
  const rate = checkRateLimit(`leads-licitacion:${ip}`, publicPerMin, 60_000);
  if (!rate.allowed) {
  ...
  const parsed = tenderLeadSchema.safeParse(body);
  if (!parsed.success) {
    return apiError(400, {
      code: 'VALIDATION_ERROR',
  ...
  try {
    const { referenceNumber } = await createTenderLead(parsed.data);
    console.info(
      JSON.stringify({
        event: 'tender_lead_created',
        referenceNumber,
      }),
    );
    ...
    console.error(
      JSON.stringify({
        event: 'tender_lead_unexpected_error',
