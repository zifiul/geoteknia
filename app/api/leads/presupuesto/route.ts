...
import { budgetLeadSchema } from '@/lib/leads/schema';
import {
  apiError,
  apiSuccess,
  zodFieldDetails,
...
export async function POST(request: NextRequest) {
  ...
  const rate = checkRateLimit(`leads-presupuesto:${ip}`, publicPerMin, 60_000);
  if (!rate.allowed) {
  ...
  const parsed = budgetLeadSchema.safeParse(body);
  if (!parsed.success) {
    return apiError(400, {
      code: 'VALIDATION_ERROR',
  ...
  try {
    const { referenceNumber } = await createBudgetLead(parsed.data);
    console.info(
      JSON.stringify({
        event: 'budget_lead_created',
        referenceNumber,
      }),
    );
    ...
    console.error(
      JSON.stringify({
        event: 'budget_lead_unexpected_error',
