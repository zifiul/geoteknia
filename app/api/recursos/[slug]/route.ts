...
import { findGatedLeadMagnetBySlug } from '@/lib/content/lead-magnets';
import { apiError, apiSuccess, zodFieldDetails } from '@/lib/http/api-envelope';
...
export async function POST(
  ...
  const rate = checkRateLimit(`recursos:${ip}`, publicPerMin, 60_000);
  if (!rate.allowed) {
  ...
  const leadMagnet = await findGatedLeadMagnetBySlug(slug);
  if (!leadMagnet) {
    return apiError(404, {
      code: 'RESOURCE_NOT_FOUND',
      message: 'El recurso solicitado no existe o no está disponible para descarga',
    });
  }
  ...
  try {
    const result = await createResourceLead(parsed.data, leadMagnet);
    console.info(
      JSON.stringify({
        event: 'resource_lead_created',
    ...
    return apiSuccess(
      {
        ...
        thankYouUrl: result.thankYouUrl,
      },
      201,
    );
  } catch (error) {
    ...
    console.error(
      JSON.stringify({
        event: 'resource_lead_unexpected_error',
        slug,
