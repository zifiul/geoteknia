...
- **Invocación uniforme:** `pnpm run security:sast` (alias legacy: `pnpm run security:semgrep`). Equivalente bash: `bash scripts/semgrep-scan.sh [base]`.
...
- **Invocación uniforme:** `pnpm run security:secrets` (alias legacy: `pnpm run security:gitleaks`). Equivalente bash: `bash scripts/gitleaks-detect.sh [base]`.
...
- **Invocación uniforme:** `pnpm run security:dast` (requiere servidor: `pnpm run dev`). Equivalente bash: `bash scripts/security-dast.sh [base] [url]`.
...
- **Invocación uniforme:** `pnpm run security:scan` — ejecuta SAST → SCA → Secretos → DAST y muestra resumen. Equivalente bash: `bash scripts/security-scan.sh [base]`.
- **Instalar herramientas:** `pnpm run security:install` (Semgrep + gitleaks en `tools/`).
...
- Herramientas: <semgrep x.y | fallback manual>, pnpm audit, <gitleaks x.y | fallback manual>, curl DAST
