/**
 * QA GTK-31 — persistencia real Neon (db-state-verify).
 */
...
import { createTenderLead } from '@/lib/leads/create-tender-lead';

const TEST_EMAIL = 'gtk31-qa@example.com';
const db = new PrismaClient();

describe('QA GTK-31 — BD licitación', () => {
  let baseline: {
    contacts: number;
    leads: number;
    projects: number;
  ...
  it('createTenderLead incrementa filas y persiste expediente/importe', async () => {
    const result = await createTenderLead({
      nombre: 'QA GTK31',
      empresa: 'QA Corp',
      email: TEST_EMAIL,
      expedienteRef: 'GTK31-QA-EXP',
      importeEstimado: 50000,
      gdprConsent: true,
      turnstileToken: 'qa-unused',
    });

    referenceNumber = result.referenceNumber;
    expect(referenceNumber).toMatch(/^LIC-/);
    ...
    expect(lead?.leadType).toBe('licitacion');
    expect(lead?.channel).toBe('formulario');
    expect(lead?.expedienteRef).toBe('GTK31-QA-EXP');
    expect(lead?.estimatedValue?.toString()).toBe('50000');
    expect(lead?.project?.expedienteRef).toBe('GTK31-QA-EXP');
    expect(lead?.project?.estimatedValue?.toString()).toBe('50000');
    expect(lead?.project?.title).toMatch(/^Licitación/);
