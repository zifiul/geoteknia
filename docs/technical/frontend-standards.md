...
- Portal admin (GTK-68+): shell, sidebar, topbar y gates en `components/organisms/admin/` (p. ej. `AdminPortalLayout`, `AdminSidebar`, `LoginForm`). Widgets de dashboard home (GTK-79) en `components/organisms/admin/dashboard/` (`KpiGrid`, `AlertsPanel`, `QuickActions`, `RecentActivity`); datos vía `lib/admin/dashboard-metrics.ts` (RSC, sin fetch cliente). Gestión de usuarios (GTK-81) en `components/organisms/admin/users/` (`UsersTable`, `UserFilters`, `CreateUserForm`, `EditUserClient`); datos y mutaciones en `lib/admin/users-queries.ts` y `lib/admin/users-actions.ts`. Listado CMS (GTK-72) en `components/organisms/admin/cms/` (`ContentTable`, `ContentFilters`, `ContentMetricsPanel`, `NewContentMenu`, `CmsStatusBadge`); catálogo y consultas en `lib/admin/cms-content-types.ts`, `lib/admin/cms-content-queries.ts`, filtros en `lib/admin/cms-filters-schema.ts`; ruta RSC `/contenido` con `loading.tsx` skeleton. Editor de contenido (GTK-73, GTK-78): `ContentEditor`, `SeoBlock`, `BodyEditor` (TipTap UI — template `simple-editor` vía CLI + `CmsTiptapUiEditor` controlado), `RelationsPicker`, `PreviewPane` en `components/organisms/admin/cms/`; componentes UI en `components/tiptap-ui/`, primitivos en `components/tiptap-ui-primitive/`, estilos SCSS en `styles/`; molécula `RichTextContent` para render público; RSC `/contenido/[type]/[id]` + skeleton; adaptadores preview en `lib/cms/preview/` (form-state → props de plantilla pública); split-view desktop / pestañas móvil (Stitch A4); mutaciones vía Server Actions GTK-41 sin `react-hook-form` (estado local + `cmsServiceFormSchema`). Generación IA en editor (GTK-74): pestaña «Generar con IA» en `ContentEditor` (`AiGeneratePanel`, `AiGenerateForm`, `AiOutputPreview`, `SectionRegenerateMenu`, `AiBudgetNotice`); `fetch` a `POST /api/admin/ia/generar`; fusión en `lib/cms/ia/apply-ai-output.ts`; UI Stitch A5; sin presupuesto en cliente (429 `BUDGET_EXCEEDED`). Flujo editorial y publicación en editor (GTK-75): `EditorialWorkflowPanel`, `WorkflowStepper`, `WorkflowActions`, `VerificationNotice`, `RevisionHistory`, `PublishDialog`, `SchedulePublishDialog`; stepper y modales Stitch A5; transiciones vía Server Actions GTK-39/40; `listContentRevisions` y programación `scheduledPublishAt` en `lib/content/revisions.ts` / `lib/content/schedule.ts`. Pipeline CRM (GTK-70) en `components/organisms/admin/crm/` (`PipelineBoard`, `ProjectList`, `CrmFilters`, `MetricsPanel`, `PipelineViewToggle`); datos en `lib/projects/queries.ts` (join `lead`), agrupación tablero en `lib/projects/board-utils.ts`; ruta `/admin/proyectos` con vista `?view=board|list` (técnico: solo lista); drag-and-drop y select «Mover a…» contra `changeStateAction` existente. Detalle de proyecto (GTK-71): mismos organismos bajo `crm/` (`ProjectHeader`, `StateChanger`, `TechnicianAssign`, `Milestones`, `Notes`, `Documents`, `CrmDetailSection`); RSC `/admin/proyectos/[id]` + `loading.tsx`; modales Radix para estado/técnico (Stitch A3); mutaciones solo vía Server Actions GTK-35 con `useTransition` + `router.refresh()`; permisos UI (`projects.assign` / `projects.delete` ocultos para técnico).
...
- Página `/licitaciones` (GTK-58): metadata en `lib/tenders/page-config.ts`; lectores `lib/content/tenders.ts` + seed `tenders-seed-data.ts`; `TenderForm` + `TurnstileWidget`; POST `/api/leads/licitacion`; enlace a `/acreditaciones`.
- Página `/acreditaciones` (GTK-59): metadata en `lib/accreditations/page-config.ts`; lector `listPublishedAccreditationsDetailed()` en `lib/content/accreditations.ts` (sin tocar `listActiveAccreditations` de la Home); `CredentialGrid`/`CredentialCard`; JSON-LD `Organization`/`hasCredential`; CTA a `/licitaciones`.
