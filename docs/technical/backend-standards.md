...
- **Pipeline CRM lectura (GTK-34):** en listados y métricas el scoping del `tecnico` va en el `where` de Prisma (`assignedTechnicianId = userId`), no post-fetch. En detalle de proyecto, tras cargar la fila, `assertOwnership`; si el técnico no es dueño, responder como recurso inexistente (`ProjectNotFoundError` / `notFound()`), no `ForbiddenError`, para anti-enumeración.
