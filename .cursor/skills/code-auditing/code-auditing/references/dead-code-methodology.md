...
```bash
pnpm add -g knip
# o usar vía pnpm dlx (sin instalación)
```
...
pnpm dlx knip --include files --exclude dependencies
```
...
3. **Testea tras la eliminación** - asegúrate de que nada se rompe (`pnpm test`, `pnpm run test:e2e`)
