...
- **THEN** se lanza un error de recurso no encontrado coherente con el patrón del proyecto (`NotFoundError` o equivalente)

#### Scenario: Tecnico ajeno al proyecto

- **WHEN** un `tecnico` solicita detalle de un proyecto existente asignado a otro técnico
- **THEN** la respuesta es indistinguible de inexistente para el usuario (mismo tratamiento que no encontrado — ver `design.md`)
...
Las páginas `app/(admin)/proyectos` SHALL ser Server Components que leen search params, validan con `projectFiltersSchema` y delegan en `lib/projects/`.
