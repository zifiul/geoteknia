...
- **WHEN** usuario `editor` invoca RFC-19 con plantilla `id=uuid_blog`, `input_params={"tema": "Riesgos sísmicos en Madrid"}`
...
- **THEN** se registra `status='error'`, `error_message='...'`, `retry_count` se incrementa. El sistema permite reintentar (RF-19 maneja reintento).
