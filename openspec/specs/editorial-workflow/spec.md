...
Máquina de estados editorial humano-en-el-bucle, versionado en `content_revisions` y Server Actions de transición para contenido YMYL publicable. Materializa GTK-39; publicación efectiva (ISR/`published_at`) en GTK-40.
...
El sistema SHALL validar en dominio que `content_type` y `content_id` referencian una entidad publicable con bloque EDITORIAL del alcance GTK-39. Entidad inexistente o soft-deleted SHALL responder como no encontrada (404).
