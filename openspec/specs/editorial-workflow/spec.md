...
Máquina de estados editorial humano-en-el-bucle, versionado en `content_revisions` y Server Actions de transición para contenido YMYL publicable. Materializa GTK-39 y GTK-40 (publicación efectiva).
...
El envelope `ContentActionResult` SHALL exponer indicación de verificación técnica obligatoria antes de aprobar tras enviar a revisión o al aprobar.
...
El sistema SHALL validar en dominio que `content_type` y `content_id` referencian una entidad publicable con bloque EDITORIAL, incluyendo `team_member`, `machinery` y los tipos GTK-39. Entidad inexistente o soft-deleted SHALL responder como no encontrada (404).
