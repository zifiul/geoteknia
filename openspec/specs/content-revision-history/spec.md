# content-revision-history Specification

## Purpose

Historial append-only de versiones editoriales de contenido publicable, con snapshot de body/SEO, estado de workflow y trazabilidad a la generación IA origen. Materializa GTK-16 y soporta RF-20/RF-21.
...
- **THEN** consulta retorna ambas versiones en milisegundos
...
- `borrador_ia` → `borrador_ia` (regeneración de la misma sección).
...
- **THEN** validación en aplicación rechaza (implementada en RF-21 backend) y la base de datos es append-only, registra solo inserciones
...
- **THEN** audit trail muestra todo el camino y cada transición es una fila nueva con `editor_id` y timestamp
