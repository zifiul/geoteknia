...
Ejecuta los cuatro chequeos invocando los **scripts pnpm uniformes** del proyecto (o sus equivalentes bash en Linux/WSL/CI). Captura la salida de cada uno para el informe `reports/security.md`.

| Chequeo | Script pnpm | Equivalente bash |
|---------|------------|------------------|
| Instalar herramientas | `pnpm run security:install` | `bash scripts/install-semgrep.sh && bash scripts/install-gitleaks.sh` |
| 1. SAST | `pnpm run security:sast` | `bash scripts/semgrep-scan.sh [base]` |
| 2. SCA | `pnpm run security:sca` (JSON: `pnpm run security:sca:json`) | `bash scripts/security-audit.sh [--json]` |
| 3. Secretos | `pnpm run security:secrets` | `bash scripts/gitleaks-detect.sh [base]` |
| 4. DAST ligero | `pnpm run security:dast` | `bash scripts/security-dast.sh [base] [url]` |
| **Todos** | `pnpm run security:scan` | `bash scripts/security-scan.sh [base]` |
...
1. **SAST:** `pnpm run security:sast` — Semgrep sobre ficheros del diff (fallback manual si falla la instalación).
2. **SCA:** `pnpm run security:sca:json` — parsea severidades y fixes; revisa dependencias nuevas en el diff.
3. **Secretos:** `pnpm run security:secrets` — gitleaks sobre commits `base..HEAD`.
4. **DAST ligero:** `pnpm run security:dast` — requiere `pnpm run dev` si hay Route Handlers en el diff; si no hay endpoints o el servidor no responde, el script lo indica (NO EJECUTADO / omitido), no declares LIMPIO.
...
Atajo: `pnpm run security:scan` ejecuta los cuatro chequeos en secuencia con resumen final; falla (exit 1) si SAST, SCA o Secretos reportan error. DAST omitido o no ejecutado no bloquea el orquestador, pero debe documentarse en el informe.
